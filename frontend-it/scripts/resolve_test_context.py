...
def write_step_summary(summary_path: str, context: dict[str, str]) -> None:
    ...
    lines = [
        ...
        f"- Resolved selector: {context['test_class'] or 'none'} ({context['test_class_source']})",
...
def main() -> None:
    ...
    if managed_issue:
        if not mapping_entry:
            validation_errors.append(f'Missing mapping entry for managed Jira issue {issue_key} in {args.mapping_path}.')
        if not mapped_test_class:
            validation_errors.append(f'Managed Jira issue {issue_key} must declare testClass in {args.mapping_path}.')
        if not mapped_execution_key:
            validation_errors.append(f'Managed Jira issue {issue_key} must declare testExecutionKey in {args.mapping_path}.')
    elif not first_non_empty(args.input_test_class, mapped_test_class):
        validation_errors.append('Non-managed runs must provide test_class explicitly; there is no default fallback selector.')
    ...
    if managed_issue:
        ...
        test_class_source = 'workflow-input' if args.input_test_class else ('mapping-file' if mapped_test_class else 'none')
        execution_key_source = 'workflow-input' if args.input_xray_test_execution_key else ('mapping-file' if mapped_execution_key else 'none')
    ...
    values = {
        ...
        'TEST_CLASS_SELECTOR': test_class,
        'XRAY_TEST_EXECUTION_KEY': execution_key,
        'XRAY_IMPORT_MODE_RESOLVED': import_mode,
        'FRONTEND_TEST_CONTEXT_VALIDATION_STATUS': validation_status,
        'FRONTEND_TEST_CONTEXT_VALIDATION_MESSAGE': validation_message,
        'FRONTEND_TEST_CONTEXT_MANAGED_ISSUE': context['managed_issue'],
    }
