...
def jira_issue(base_url: str, email: str, token: str, issue_key: str) -> dict[str, object]:
    ...
    response_bytes = http_request(
        url=f"{base_url.rstrip('/')}/rest/api/3/issue/{encoded_issue_key}?fields=issuetype",
        method="GET",
        headers={
...
def authenticate_xray(xray_base_url: str, client_id: str, client_secret: str) -> str:
    response_bytes = http_request(
        url=f"{xray_base_url.rstrip('/')}/api/v2/authenticate",
        method="POST",
        headers={"Content-Type": "application/json"},
        data=json.dumps({"client_id": client_id, "client_secret": client_secret}).encode("utf-8"),
    )
...
def graphql_request(
    ...
    response_bytes = http_request(
        url=f"{xray_base_url.rstrip('/')}/api/v2/graphql",
        method="POST",
        headers={
...
def summary_text(result: dict[str, object]) -> str:
    return "\n".join(
        [
            ...
            f"- Existing tests before sync: {', '.join(result.get('existing_test_keys', [])) or 'none'}",
            f"- Added tests: {', '.join(result.get('added_test_keys', [])) or 'none'}",
            "",
        ]
    )
...
def main() -> int:
    ...
    try:
        ...
        if not jira_user_email or not jira_api_token:
            result["message"] = "Skipping Xray execution member sync because JIRA_USER_EMAIL / JIRA_API_TOKEN are not configured."
            write_text(summary_path, summary_text(result))
            write_json(result_path, result)
            print(str(result["message"]))
            return 0
        ...
        if not xray_client_id or not xray_client_secret:
            result["message"] = "Skipping Xray execution member sync because XRAY_CLIENT_ID / XRAY_CLIENT_SECRET are not configured."
            write_text(summary_path, summary_text(result))
            write_json(result_path, result)
            print(str(result["message"]))
            return 0
        ...
        execution_issue_type = str(
            ((execution_issue.get("fields") or {}).get("issuetype") or {}).get("name", "")
        ).strip()
        if execution_issue_type != "Test Execution":
            raise RuntimeError(
                f"{execution_issue_key} is a Jira issue of type {execution_issue_type or 'unknown'}, not a Test Execution."
            )
        ...
        for test_issue_key in mapped_test_keys:
            ...
            issue_id = str(jira_test_issue.get("id", "")).strip()
            issue_type = str(((jira_test_issue.get("fields") or {}).get("issuetype") or {}).get("name", "")).strip()
            if issue_type != "Test":
                raise RuntimeError(f"{test_issue_key} is a Jira issue of type {issue_type or 'unknown'}, not a Test.")
            test_issue_ids[test_issue_key] = issue_id
        ...
        current_response = graphql_request(
            ...
            xray_token,
            """
            query GetExecutionTests($issueId: String!, $limit: Int!) {
        ...
        result["existing_test_keys"] = sorted(
            reverse_test_issue_ids[issue_id] for issue_id in current_issue_ids if issue_id in reverse_test_issue_ids
        )

        missing_issue_ids = [issue_id for issue_id in test_issue_ids.values() if issue_id not in current_issue_ids]
        ...
        if missing_issue_ids:
            ...
            warning_text = str(
                (((mutation_response.get("data") or {}).get("addTestsToTestExecution") or {}).get("warning", ""))
            ).strip()
            result["message"] = (
                f"Synced mapped Jira Tests into {execution_issue_key}: {', '.join(missing_issue_keys)}."
